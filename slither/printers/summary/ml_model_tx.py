...
class ModelPredictionTx(AbstractPrinter):
    """
    Predictor Based On Machine Learning Models
    """

    ARGUMENT = 'model-prediction-tx'
    HELP = 'Model Prediction For Tx-origin Vulnerability'
    ...
    def __init__(self, slither, logger):
        super(ModelPredictionTx, self).__init__(slither, logger)
...
class MLModelTx(object):
    def __init__(self):
        self._model = self._load_trained_model()

    def _load_trained_model(self):
        url = "https://drive.google.com/uc?id=1MyujOvsJFbABtydxKmWgwXHti4V-vldB"
        temp_path = tempfile.gettempdir()
        if not os.path.exists(os.path.join(temp_path, 'eec_tx.joblib')):
            gdown.download(url, os.path.join(temp_path, 'eec_tx.joblib'), quiet=True)
        return joblib.load(os.path.join(temp_path, 'eec_tx.joblib'))
