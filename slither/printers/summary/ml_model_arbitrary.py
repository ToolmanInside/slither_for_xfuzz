...
class ModelPredictionArbitrary(AbstractPrinter):
    """
    Predictor Based On Machine Learning Models
    """

    ARGUMENT = 'model-prediction-arbitrary'
    HELP = 'Model Prediction For Reentrancy Vulnerability'
    ...
    def __init__(self, slither, logger):
        super(ModelPredictionArbitrary, self).__init__(slither, logger)
    ...
    def calculate_func_prior(self, function, model_predict):
        ...
        parameter_weights = 0
        predict_factor = 0.5 if model_predict else 1
        weight_dict = {
            'address': 1,
            'uint': 1.5,
            'bytes': 2,
...
class MLModelArbitrary(object):
    def __init__(self):
        self._model = self._load_trained_model()

    def _load_trained_model(self):
        url = "https://drive.google.com/uc?id=1R5MtAqLHKlHfcBbPgvMXQqKy_GLFbGPX"
        temp_path = tempfile.gettempdir()
        if not os.path.exists(os.path.join(temp_path, 'eec_clf.joblib')):
            gdown.download(url, os.path.join(temp_path, 'eec_clf.joblib'), quiet=True)
        # if not os.path.exists("eec_clf.joblib"):
        #     # TODO: throw error
        #     raise ValueError("predict model not found")
        return joblib.load(os.path.join(temp_path, 'eec_clf.joblib'))
